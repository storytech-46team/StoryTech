...
Для генерации контента пользователь должен выбрать курс, модуль и занятие.

Система отображает структуру выбранного пользователем занятия, пользователь должен выбрать конкретные материалы для генерации новеллы.

Далее пользователь должен сделать выбор миров и персонажей.
...
Система должна отображать сгенерированную структуру в виде дерева, где каждая Глава содержит несколько Сцен (например, Глава 1: Сцена 1, Сцена 2, Сцена 3)

Сцены должны содержать текст (диалоги) и мизансцену (описание для генерации изображений)
...
Предусмотреть возможность редактирование  диалога к сцене в отдельном окне Редактора диалогов (п.6).
...
После утверждения  всех Глав, пользователь должен иметь возможность одобрить содержание новеллы с помощью кнопки "Одобрить содержание". Система должна сохранить содержание и перейти к следующему к этапу Предпросмотр.
...
В MVP предусмотрен  только просмотр результата генерации
...
Навигация по главам, сценам и страницам внутри сцен: Пользователь должен иметь возможность навигации по Главам и Сценам с помощью кнопок. Система должна отображать текущую страницу для выбранной Главы и Сцену (например, Глава 1, Сцена 2, Страница 1).
