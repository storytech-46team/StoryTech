...
1. [Описание продукта](#Описание-продукта)
    ...
    - [Пример структуры новеллы](#Пример-структуры-новелы)
...
5. [Коды генерации новеллы](#Коды-генерации-новеллы)
...
Скринфлоу
![2AdGbWVP-vhtOG135l](https://github.com/user-attachments/assets/f0e39f5c-e7be-402b-a6f6-3100b15128f7)
...
В данном разделе приведен пример таблиц, а так же необходиые sql запросы для их создания.
...
| Метод                          | Описание                                                                                   | Результат вызова                                                                                                                                                                                                                       |
|--------------------------------|-------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `select_novel_theme`           | Выбор темы новеллы                                                                         | В атрибут `theme` класса `novella` запишется тема                                                                                                                                                                                     |
| `select_location`              | Выбор локации                                                                              | В атрибут `location` класса `novella` будет записана локация. MVP считаем, что 1 локация всего. Надо в пайплайне сделать раздельные блоки выбора локации и персонажей. Программно это будет разная реализация. Имеет смысл соответствия пайплайна с кодом. Тут в коде я уже разделил |
| `select_characters`            | Выбор персонажей                                                                           | В атрибут `character` класса `novella` будут записаны 1-2 персонажа. MVP считаем, что максимум два персонажа                                                                                                                         |
| `summarize_material`           | Суммаризация учебного материала                                                            | Модель LLM разделяет учебный материал на составные части (главы), создавая краткую выжимку, для генерации дальнейшего промта/сценария                                                                                                 |
| `split_into_chapters`          | Разделение предварительного сценария на главы                                              | После суммаризации происходит разделение на главы. Главы записываются в соответствующий словарь. Ключ - номер блока, Значение - экземпляр класса `chapter`                                                                             |
| `generate_pre_scenario`        | Генерация предварительного сценария согласно краткой выжимке и разделению на блоки         | В атрибут `pred_scenario` класса `novella` будет записан предварительный сценарий. Так же в атрибут `promt` класса `novella` будет записан промт                                                                                      |
| `edit_materials`               | Редактирование материалов: сценарий, промт                                                 | Обновление атрибутов класса `novella` актуальными версиями сценария и промта. ??? Возможно имеет смысл сделать два разных метода                                                                                                      |
| `save_scenario`                | Одобрение и сохранение сценария                                                            | Что такое одобрение? Как это реализовать программно. Понятно, что в базу запишется сценарий, и запишется в атрибут класса. Но это сохранение. Что такое одобрение?                                                                    |
| `generate_chapter_prompt`      | Генерация промта для глав                                                                   | Сгенерированные промты для каждого из глав. Во всех экземплярах класса `chapter` должны быть заполнены атрибуты `promt`. Единственное: на вход надо передавать количество блоков, которое выбрал новеллист                                |
| `generate_chapters`            | Генерация глав по алгоритму сценария                                                       | Сгенерированный текст глав должен быть записан в атрибут `text` у всех объектов класса `chapter`                                                                                                                                      |
| `generate_scenes`              | Генерация сцен и мизансцен                                                                 | Согласно количеству сгенерированных сцен в объекте класса `chapter` должен быть создан словарь: ключ - номер сцены, значение - экземпляр класса `scene`                                                                               |
| `save_chapters_to_db`          | Сохранение разделов в БД                                                                   |                                                                                                                                                                                                                                       |
| `save_scenes_to_db`            | Сохранение сцен в БД                                                                       |                                                                                                                                                                                                                                       |
| `select_chapter`               | Выбор раздела                                                                              | Выбор главы по номеру                                                                                                                                                                                                                |
| `preview_chapter`              | Предпросмотр главы                                                                         | Предпросмотр выводится в ячейку                                                                                                                                                                                                      |
...
## Требования
Требования к проекту, которые необходимы для его воспроизведения.

## Аппаратные требования

### 1. Минимальные характеристики:

| Компонент (CPU) | Требования | Пример |
|-----------------|------------|--------|
| Процессор (CPU) | от 8 ядер 16 потоков, от 3.5 ГГц, кэш от 16МБ | Intel Xeon E5 2620 v4 (3 GHz / 8 ядер 16 потоков / 20 MB), AMD Ryzen 7 3700X (4.4 GHz / 8 ядер 16 потоков / 32 MB) |
| Видеокарта (GPU) | от 12ГВ RAM на GPU с CUDA или OpenGL | NVIDIA GeForce RTX 3090 Ti (24 ГБ GDDR6X) |
| ОЗУ | от 32 ГБ DDR4 или DDR5 RAM | |
| Хранение | SSD на 512 ГБ | NVMe предпочтительно |
...
| Компонент (CPU) | Требования | Пример |
|-----------------|------------|--------|
| Процессор (CPU) | 16 ядер 32 потока, от 3.7ГГц, кэш от 32МБ | Intel Xeon Gold 6346 (3.6 GHz / 16 ядер 32 потока / 36 МБ), AMD Ryzen 9 5900X (4.8 GHz / 16 ядер 32 потока / 64 МБ) |
| Видеокарта (GPU) | от 24ГВ VRAM с CUDA или OpenGL | NVIDIA RTX A5000 (24 ГБ GDDR6), NVIDIA GeForce RTX 4090 (24 ГБ GDDR6X) |
| ОЗУ | 32-64 ГБ DDR4 или DDR5 RAM | |
| Хранение | SSD на 1 ТБ | NVMe предпочтительно |
...
| Компонент (CPU) | Требования | Пример |
|-----------------|------------|--------|
| Процессор (CPU) | от 16 ядер 32 потоков, от 4.1 ГГц, кэш от 32МБ | Intel Xeon Gold 6544Y (4.1 GHz / 16 ядер 32 потока / 45 МБ), AMD EPYC 9174F (4.4 GHz / 16 ядер 32 потока / 32 МБ) |
| Видеокарта (GPU) | профессиональная видеокарта с большой VRAM | NVIDIA A100 (80 ГБ HBM2e), NVIDIA Quadro RTX A6000 (48 ГБ) |
| ОЗУ | 128-256 ГБ DDR4 или DDR5 RAM | |
| Хранение | массив SSD на 2-4 ТБ | NVMe предпочтительно |

## Программные требования

### ComfyUI - Генерация изображений по API
Генерация изображений происходит на нашем сервере, для этого необходимо развернуть ComfyUI, закинуть модели в соответствующие папки и можно будет использовать генерацию изображений по API.
...
- [Шаблон python запроса изображения по API](flux/template-flux-api-in-python.py) (в финальном проекте данный код доработан, это лишь пример базового шаблона запроса изображения по API)
- [Визуальное отображение взаимосвязи модулей flux](flux/flux.dev1.node-pipeline.png)

Также для генерации изображений в нашем проекте используется Misral для перевода текста с русского на английский.


### Генерация текстов по API
Для генерации текстов в нашем проекте используется GPT-4o от OpenAI по API, но с использованием прокси, который указывается в питон коде.
Необходимо:
1. API от OpenAI для GPT-4o
2. Прокси, который будет позволять подключаться к GPT-4o от OpenAI



## Код генерации новеллы

Вся генерация в нашем проекте происходит в jupyter notebook, что позволяет при необходимости перенести коды в python скрипты и собрать в дальнейшем полноценное веб приложение.
Весь проект от генерации картинок, текста и до объединения всего в один документ происходит в одной тетратке юпитер ноутбука.

Сама тетрадка [NovelGenetrator.ipynb](ipynb/NovelGenerator.ipynb)

Подбробно в человекочитаемом виде с кодом можно ознакомиться по следующей ссылке 
[Генерация новеллы_в формате jupyter notenook](NovelGenerator.md)
